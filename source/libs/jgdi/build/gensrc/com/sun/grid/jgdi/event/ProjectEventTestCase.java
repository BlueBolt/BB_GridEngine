...
public class ProjectEventTestCase extends BaseTestCase {

   private JGDI jgdi;
   private EventClient evc;
   
   public ProjectEventTestCase(String name) {
      super(name);
   }
   
   public static Test suite() {
      TestSuite suite = new TestSuite(ProjectEventTestCase.class);
      return suite;
   }
   ...
   public void testProjectEvents() throws Exception {
      ...
      assertTrue("No test values for Project", testValues.length > 0 );
      
      Project testObj = (Project)testValues[0];
      ...
      try {
          ...
          map.put(EventTypeEnum.ProjectAdd, flushInterval);
          map.put(EventTypeEnum.ProjectDel, flushInterval);
         
         
          map.put(EventTypeEnum.ProjectMod, flushInterval);
         ...
         assertTrue("Project: Did not get the add event", waitThread.waitForAddEvent(timeout));

         
         // @jgdi.todo modify the test object

         jgdi.updateProject(testObj);
         logger.fine(testObj + " modified");
         assertTrue("Project: Did not get the mod event", waitThread.waitForModEvent(timeout));
         ...
         assertTrue("Project: Did not get the del event", waitThread.waitForDelEvent(timeout));

...
public class UserTestCase extends BaseTestCase {
   ...
   public  UserTestCase(java.lang.String testName) {
      super(testName);
   }
   
   public static Test suite() {
      TestSuite suite = new TestSuite(UserTestCase.class);
      return suite;
   }
   
  
   
   public void testUserAddGetDelete() throws Exception {
   
      Object[] testValues = TestValueFactory.getTestValues(User.class);
      assertTrue("We have no test values for User", (testValues.length > 0));

      JGDI jgdi = createJGDI();
      try {
         List<Util.Difference> differences = new LinkedList<Util.Difference>();
         for (int i = 0; i < testValues.length; i++) {
            User testObj = (User)testValues[i];
            logger.info("adding testObj " + testObj);
            jgdi.addUser(testObj);
            try {
              logger.info("fetching and diffing testObj " + testObj);
              User retObj = jgdi.getUser(testObj.getName());
              
              Util.getDifferences(testObj, retObj, differences);
              if (!differences.isEmpty()) {
                 logger.warning("org User is not equal to stored User ------------------------"); 
   ...
   public void testUserListXML() throws Exception {
      JGDI jgdi = createJGDI();
      try {
         for (User obj : jgdi.getUserList()) {
            File file = File.createTempFile("User", ".xml");
            try {
               XMLUtil.write(obj, file);
               User obj1 = (User)XMLUtil.read(file);
               
// begin debug: check differences
//               List<Util.Difference> differences = new LinkedList<Util.Difference>();
//               Util.getDifferences(obj, obj1, differences);
//               if (!differences.isEmpty()) {
//                  logger.warning("org User is not equal to filed/reread obj1 User ------------------------"); 
//                  for (Util.Difference diff : differences) {
//                     logger.warning(diff.toString());
//                  }
//               }
...
// end debug: check differences
               ...
               assertTrue("serialized xml object of class User is invalid", 

...
public class ConfigurationTestCase extends BaseTestCase {
   ...
   public  ConfigurationTestCase(java.lang.String testName) {
      super(testName);
   }
   
   public static Test suite() {
      TestSuite suite = new TestSuite(ConfigurationTestCase.class);
      return suite;
   }
   
  
   
   public void testConfigurationAddGetDelete() throws Exception {
   
      Object[] testValues = TestValueFactory.getTestValues(Configuration.class);
      assertTrue("We have no test values for Configuration", (testValues.length > 0));

      JGDI jgdi = createJGDI();
      try {
         List<Util.Difference> differences = new LinkedList<Util.Difference>();
         for (int i = 0; i < testValues.length; i++) {
            Configuration testObj = (Configuration)testValues[i];
            logger.info("adding testObj " + testObj);
            jgdi.addConfiguration(testObj);
            try {
              logger.info("fetching and diffing testObj " + testObj);
              Configuration retObj = jgdi.getConfiguration(testObj.getName());
              
              Util.getDifferences(testObj, retObj, differences);
              if (!differences.isEmpty()) {
                 logger.warning("org Configuration is not equal to stored Configuration ------------------------"); 
              ...
              jgdi.deleteConfiguration(testObj);
   ...
   public void testConfigurationListXML() throws Exception {
      JGDI jgdi = createJGDI();
      try {
         for (Configuration obj : jgdi.getConfigurationList()) {
            File file = File.createTempFile("Configuration", ".xml");
            try {
               XMLUtil.write(obj, file);
               Configuration obj1 = (Configuration)XMLUtil.read(file);
               
// begin debug: check differences
//               List<Util.Difference> differences = new LinkedList<Util.Difference>();
//               Util.getDifferences(obj, obj1, differences);
//               if (!differences.isEmpty()) {
//                  logger.warning("org Configuration is not equal to filed/reread obj1 Configuration ------------------------"); 
//                  for (Util.Difference diff : differences) {
//                     logger.warning(diff.toString());
//                  }
//               }
...
// end debug: check differences
               ...
               assertTrue("serialized xml object of class Configuration is invalid", 

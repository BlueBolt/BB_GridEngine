 ...
 *
 */
public class ProjectTestCase extends BaseTestCase {
   
   public  ProjectTestCase(java.lang.String testName) {
      super(testName);
   }
   
   public static Test suite() {
      TestSuite suite = new TestSuite(ProjectTestCase.class);
      return suite;
   }
   
  
   
   public void testProjectAddGetDelete() throws Exception {
   
      Object[] testValues = TestValueFactory.getTestValues(Project.class);
      assertTrue("We have no test values for Project", (testValues.length > 0));

      JGDI jgdi = createJGDI();
      try {
         List<Util.Difference> differences = new LinkedList<Util.Difference>();
         for (int i = 0; i < testValues.length; i++) {
            Project testObj = (Project)testValues[i];
            logger.info("adding testObj " + testObj);
            jgdi.addProject(testObj);
            try {
              logger.info("fetching and diffing testObj " + testObj);
              Project retObj = jgdi.getProject(testObj.getName());
              
              Util.getDifferences(testObj, retObj, differences);
              if (!differences.isEmpty()) {
                 logger.warning("org Project is not equal to stored Project ------------------------"); 
   ...
   public void testProjectListXML() throws Exception {
      JGDI jgdi = createJGDI();
      try {
         for (Project obj : jgdi.getProjectList()) {
            File file = File.createTempFile("Project", ".xml");
            try {
               XMLUtil.write(obj, file);
               Project obj1 = (Project)XMLUtil.read(file);
               
// begin debug: check differences
//               List<Util.Difference> differences = new LinkedList<Util.Difference>();
//               Util.getDifferences(obj, obj1, differences);
//               if (!differences.isEmpty()) {
//                  logger.warning("org Project is not equal to filed/reread obj1 Project ------------------------"); 
//                  for (Util.Difference diff : differences) {
//                     logger.warning(diff.toString());
//                  }
//               }
...
// end debug: check differences
               ...
               assertTrue("serialized xml object of class Project is invalid", 

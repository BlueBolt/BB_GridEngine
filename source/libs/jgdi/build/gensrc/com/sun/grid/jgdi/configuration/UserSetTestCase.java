 ...
 *
 */
public class UserSetTestCase extends BaseTestCase {
   
   public  UserSetTestCase(java.lang.String testName) {
      super(testName);
   }
   
   public static Test suite() {
      TestSuite suite = new TestSuite(UserSetTestCase.class);
      return suite;
   }
   
  
   
   public void testUserSetAddGetDelete() throws Exception {
   
      Object[] testValues = TestValueFactory.getTestValues(UserSet.class);
      assertTrue("We have no test values for UserSet", (testValues.length > 0));

      JGDI jgdi = createJGDI();
      try {
         List<Util.Difference> differences = new LinkedList<Util.Difference>();
         for (int i = 0; i < testValues.length; i++) {
            UserSet testObj = (UserSet)testValues[i];
            logger.info("adding testObj " + testObj);
            jgdi.addUserSet(testObj);
            try {
              logger.info("fetching and diffing testObj " + testObj);
              UserSet retObj = jgdi.getUserSet(testObj.getName());
              
              Util.getDifferences(testObj, retObj, differences);
              if (!differences.isEmpty()) {
                 logger.warning("org UserSet is not equal to stored UserSet ------------------------"); 
   ...
   public void testUserSetListXML() throws Exception {
      JGDI jgdi = createJGDI();
      try {
         for (UserSet obj : jgdi.getUserSetList()) {
            File file = File.createTempFile("UserSet", ".xml");
            try {
               XMLUtil.write(obj, file);
               UserSet obj1 = (UserSet)XMLUtil.read(file);
               
// begin debug: check differences
//               List<Util.Difference> differences = new LinkedList<Util.Difference>();
//               Util.getDifferences(obj, obj1, differences);
//               if (!differences.isEmpty()) {
//                  logger.warning("org UserSet is not equal to filed/reread obj1 UserSet ------------------------"); 
//                  for (Util.Difference diff : differences) {
//                     logger.warning(diff.toString());
//                  }
//               }
...
// end debug: check differences
               ...
               assertTrue("serialized xml object of class UserSet is invalid", 
